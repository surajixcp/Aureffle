        ...
        <>
            ...
            <motion.div
                ...
                className="fixed inset-0 w-full min-h-screen sm:h-[90vh] md:h-[600px] backdrop-blur-lg 
                          rounded-none sm:rounded-lg md:rounded-xl overflow-hidden z-50 bg-black/80"
            >
                {/* Main Content */}
                        ...
                        <AnimatePresence mode="wait">
                            <motion.div
                                ...
                                className="relative w-full aspect-[16/9] max-w-[95%] sm:max-w-[85%] md:max-w-3xl 
                                         h-auto max-h-[70vh] rounded-lg overflow-hidden shadow-2xl border border-[#c6a252]/40 bg-[#080d1a]"
                                ...
                                onClick={onClose}
                            >
                                <MediaItem item={selectedItem} className="w-full h-full object-contain bg-black/60" onClick={onClose} />
                                <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 md:p-5 
                                              bg-gradient-to-t from-black/90 via-black/50 to-transparent">
                                    <h3 className="text-[#f4ecce] text-base sm:text-lg md:text-xl font-semibold">
                                        {selectedItem.title}
                                    </h3>
                                    <p className="text-[#ded5c0]/85 text-xs sm:text-sm mt-1 font-light">
                                        {selectedItem.desc}
                                    </p>
                                </div>
                            </motion.div>
                ...
                {/* Close Button */}
                <motion.button
                    className="absolute top-3 sm:top-4 md:top-5 right-3 sm:right-4 md:right-5 
                              p-2.5 rounded-full bg-[#0c1426]/90 text-[#ffe600] border border-[#ffe600]/40 hover:bg-[#ffe600]/20 
                              text-xs sm:text-sm backdrop-blur-md cursor-pointer z-50 shadow-lg"
                    onClick={onClose}
            ...
            >
                <motion.div
                    className="relative rounded-xl bg-[#0c1426]/90 backdrop-blur-xl 
                             border border-[#c6a252]/40 shadow-2xl
                        ...
                        {mediaItems.map((item, index) => (
                            <motion.div
                                ...
                                className={`
                                    ...
                                    ${selectedItem.id === item.id
                                        ? 'ring-2 ring-[#c6a252] shadow-lg scale-110'
                                        : 'hover:ring-2 hover:ring-[#c6a252]/50'}
                                `}
                            ...
                            >
                                ...
                                {selectedItem.id === item.id && (
                                    <motion.div
                                        layoutId="activeGlow"
                                        className="absolute -inset-2 bg-[#c6a252]/30 blur-xl"
                                        initial={{ opacity: 0 }}
                                        animate={{ opacity: 1 }}
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-5xl">
            <div className="mb-8 text-center">
                <motion.h1
                    className="text-2xl sm:text-3xl md:text-4xl font-serif font-normal text-[#fcfbf9]"
                ...
                <motion.p
                    className="mt-2 text-sm sm:text-base text-[#ded5c0]/80 font-light max-w-xl mx-auto"
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
            ...
            <AnimatePresence mode="wait">
                {selectedItem ? (
                    ...
                    <motion.div
                        className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-3.5 auto-rows-[140px] sm:auto-rows-[160px]"
                        initial="hidden"
                        animate="visible"
                        exit="hidden"
                        variants={{
                    ...
                    >
                        {items.map((item, index) => (
                            <motion.div
                                ...
                                className={`relative overflow-hidden rounded-xl border border-[#c6a252]/20 hover:border-[#c6a252]/60 cursor-move shadow-xl ${item.span}`}
                            ...
                            >
                                ...
                                >
                                    <div className="absolute inset-0 flex flex-col justify-end p-2 sm:p-3 md:p-4">
                                        ...
                                        <p className="relative text-[#ded5c0]/80 text-[10px] sm:text-xs md:text-sm mt-0.5 line-clamp-2">
                                            {item.desc}
                                        </p>
                                    </div>
                                </motion.div>
